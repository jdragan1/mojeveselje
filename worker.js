...
async function handleInvitePost(request, env) {
  ...
  if (session) await addToUserIndex(env, session.email, id);
...
async function handleMyInvites(request, env) {
  if (!env.INVITES) return json({ error: 'KV baza (INVITES) nije povezana.' }, 500);
  const session = await getSession(request, env);
  if (!session) return json({ error: 'Niste prijavljeni.' }, 401);
  const raw = await env.INVITES.get('user-invites:' + session.email);
  let ids = [];
  ...
  for (const id of ids) {
    ...
    items.push({
      id, title: invite.title || '(bez naslova)', tpl: invite.tpl, style: invite.style,
      active: !!invite.active, hostToken: invite.hostToken, createdAt: invite.createdAt || 0,
      updatedAt: invite.updatedAt || 0, hasCover: !!(invite.cover && invite.cover.startsWith('data:')),
      isCanva: !!(invite.canva && invite.canva.enabled), views
    });
  }
...
//
// PRIVREMENO (ukloniti pre prodaje!): "SVADBA2026" je hardkodovan ovde kao dodatni
// univerzalni test kod, nezavisan od Worker secret-a, da bi testiranje bilo moguce
// odmah bez dodatnog podesavanja. Ne cuva se u bazi, ne trosi se.
const HARDCODED_TEST_CODE = 'SVADBA2026'; // TODO: ukloniti pre puštanja u prodaju
