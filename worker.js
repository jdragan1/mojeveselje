...
async function handleInviteDelete(request, env) {
  ...
  const isAdmin = !!(session && session.role === 'admin');
  if (!isOwner && !isAdmin) return json({ error: 'Nemate dozvolu da obrišete ovu pozivnicu.' }, 403);
...
async function handleRsvpPost(request, env) {
  ...
  const inviteRaw = await env.INVITES.get('invite:' + id);
  if (!inviteRaw) return json({ error: 'Pozivnica ne postoji.' }, 404);
  // Strukturirani zapis: clanovi je niz {ime, obrok} objekata — po jedna osoba po stavci.
  ...
  const clanovi = Array.isArray(body.clanovi)
    ? body.clanovi.map(c => ({ ime: String((c && c.ime) || '').slice(0, 100), obrok: String((c && c.obrok) || '').slice(0, 40) })).filter(c => c.ime).slice(0, 30)
    : [];
  const rsvp = {
