...
async function renderFileThemeHTML(request, env, id, invite) {
  ...
  try {
    ...
    const values = {
      title: invite.title || 'Pozivnica',
      date: dateFormatted,
      date_label: dateFormatted ? '' : '',
      time_suffix: invite.time ? (' u ' + invite.time + 'h') : '',
      message: invite.message || '',
      cover_image: invite.cover ? (invite.cover.startsWith('data:') ? origin + '/api/cover?id=' + id + '&src=cover' : invite.cover) : '',
...
async function routeRequest(request, env, ctx, url, pathname, method) {
    ...
    if (ct.includes('text/html')) {
      const noCacheResp = new Response(assetResp.body, assetResp);
